...
{
    public static String URL => "http://127.0.0.1:6800/jsonrpc";
...
public class Aria2NetClient
{
    ...
    public static async Task<IList<DownloadStatusResult>> CopyCompleteAll()
    {
        ...
        foreach (var item in result)
        {
            ...
            if (tt.Status.Equals("complete"))
                foreach (var file in item.Files)
                {
                    ...
                    {
                        System.IO.File.Move(file.Path, Path.Combine(VHSPath, fileName), true);
                    }
                }
        }
